...
def readData(dir_path, per):
    ...
    for root, sdirs, files in os.walk(dir_path):
        print root,sdirs,root.find("__MACOSX"),'\n'
        for filename in files:
            file_path = os.path.join(root, filename)
            #print root,sdirs,root.endswith('ham'),'\n'
            if filename.endswith('.txt') and root.find("__MACOSX") <1:
                flag = 'unknown'
                if filename.endswith('.ham.txt') or root.endswith('ham'):
                    flag = 'ham'
                    y=-1
                elif filename.endswith('.ham.txt') or root.endswith('spam'):
                    flag = 'spam'
                    y=1
                else:
                    print ('[ERROR]', flag,root, filename)
                    continue
                # print flag,filename
                spl_char = '!%^_`{|}~"#$&\'()*+,-./:;<=>@[\\]\n'
